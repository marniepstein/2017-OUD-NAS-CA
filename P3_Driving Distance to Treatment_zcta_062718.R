######################################################
# Program: P3_Driving Distance to Treatment_zcta_062718
# Author: Marni Epstein
# Date: March 2021
# Description: This program returns the driving distance and time from ZCTA centroids to treatment, using the Google API
######################################################
...
#Download package from github instead of CRAN because it fixes this issue: https://github.com/rodazuero/gmapsdistance/issues/17
...
# Set API key
set.api.key("XXXXXXXXXXXXXXXXXXXXXXX") # Set API key. Previously used one connected to Marni Epstein's credit card. Can create one through any google account
...
data <- read_csv("zcta_res.csv") ## CHANGE file name
...
data <- data %>%
  ...
  mutate(centroid = paste(zlat,"+",zlon, sep = ""))
...
#test
#geocodedrive(small$latlong1, small$time1, small$dist1)
...
# If any of them don't work, re-run here. CHANGE 

#Look to see which zip codes are missing
test <- filter(data, is.na(data$time1))

#Only keep non-missing
data <- filter(data, !is.na(data$time1))
...
write_csv(data, "Top 10 distances geocded/zcta_res_complete_geodrive.csv") # CHANGE file name

#Keep only county and 3 closest time/distance variables
# rename new name = old name
# CHANGE names for each type of treatment location
final <- data %>%
  select(zcta,  # closetreatid1, closetreatid2, closetreatid3, 
         lowtime1_res = lowtime1,
         lowtime2_res = lowtime2,
         lowtime3_res = lowtime3,
         lowdist1_res = lowdist1,
         lowdist2_res = lowdist2,
         lowdist3_res = lowdist3,
         closest1_res = closest1,
         closest2_res = closest2,
         closest3_res = closest3)

write_csv(final, "zcta_res_geodrive.csv") #CHANGE filename
